...
void MarkovChain::addObservation(const state_sequence& prevState, state_single currentState) {
	...
	//We check if the sequence of states prevState is valid
	if (!validateStateSequence(prevState)){
		return;
	}

	//Transform the sequence of previous states to string and call it key
	state_single key = stateSequenceToString(prevState);
	bool have_key = true;

	try { //checkin if we have this key
		model.at(key); 
	} catch (const std::out_of_range& oor) {
		have_key = false;
	}

	if (have_key) { //if we have the key, the new observation is added on that key
		model[key].push_back(currentState);
	} else { //if we dont have the key, create a new sequence of states
...
state_single MarkovChain::generateObservation(const state_sequence& prevState, int maxOrderWanted, bool needChoice) {
	...
	if (model.size() == 0) //cuando empezamos el modelo basicamente
	{
	...
	state_sequence poss_next_states{}; //Give me the possible next states
	bool have_key = true;
...
bool MarkovChain::fromString(const std::string& savedModel) {
	...
	// at this point, we hope something was loaded
	return true;
